...
The Hopper offline RL datasets (10 source replay buffers + 1 target replay buffer, 1.15 GB total HDF5) are **not in this git repo** because each file exceeds GitHub's 100 MB per-file limit. Users can regenerate the datasets from scratch using `Hopper_Gravity/src/collect_target_sac.py` for the target and analogous SAC runs at integer gravities g=1..10 for the sources (~5 hours total).
